...
export async function GET() {
  try {
    ...
    // 6. SQL Views - Eseguiamo in step separati per evitare problemi di scope e dipendenze
    const SQL_STEPS = [
      `DROP VIEW IF EXISTS classifica_complessiva CASCADE`,
      `DROP VIEW IF EXISTS classifica_finale_disciplina CASCADE`,
      `DROP VIEW IF EXISTS classifica_qualificazione_disciplina CASCADE`,
      `DROP VIEW IF EXISTS v_participations CASCADE`,

      // 1. Vista Base Partecipazioni
      `CREATE VIEW v_participations AS
       SELECT 
         ...
         CASE WHEN ms.points > (SELECT points FROM match_sides WHERE match_id = m.id AND side != ms.side LIMIT 1) THEN 1 ELSE 0 END AS is_win,
         m.played_at
       FROM match_side_athletes msa
       JOIN match_sides ms ON ms.id = msa.side_id
       JOIN matches m ON m.id = ms.match_id`,

      // 2. Classifica Qualificazione
      `CREATE VIEW classifica_qualificazione_disciplina AS
       WITH athlete_stats AS (
         SELECT athlete_id, discipline_id, COUNT(*) as total_matches
      ...
      // 3. Classifica Finale per Disciplina (Somma Fase 1 + Fase 2)
      `CREATE VIEW classifica_finale_disciplina AS
       ...
       LEFT JOIN athlete_match_scores ams ON ams.athlete_id = a.id AND ams.discipline_id = d.id
       GROUP BY a.id, d.id, d.kind, d.name`,

      // 4. Classifica Complessiva Torneo
