...
export async function GET() {
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (let roundIdx = 0; roundIdx < SCHEDULE_DATA.length; roundIdx++) {
        ...
        for (let slotIdx = 0; slotIdx < roundData.length; slotIdx++) {
          ...
          const turn = await tx.qualificationTurn.create({
          ...
          for (const item of disciplineMapping) {
            ...
            if (!disc) continue;
            await tx.qualificationSlot.create({
              data: {
                turnId: turn.id,
    ...
    return NextResponse.json({ ok: true, message: "Bootstrap completato con successo", ...result });
  } catch (error: any) {
